...
const TableMaterial = (props: ITableCategoryProps) => {
  const { keyword } = props;
  ...
  const { data, refetch } = useQuery(
    ['get-table-material-list', params],
    () => {
      return MaterialPageApi.getMaterialList({
  ...
  const { mutate: deleteColumn } = useMutation(
    (id: number | string) => MaterialPageApi.deleteMaterial(id),
    {
      onSuccess(res) {
        if (res?.code === 200) {
