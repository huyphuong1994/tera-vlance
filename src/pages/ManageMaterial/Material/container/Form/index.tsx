...
import MaterialPageApi from '../../_api';
...
const MaterialForm = (props: IProps) => {
  ...
  const {
    ...
    resolver: yupResolver<IFormMaterial>(schema),
    mode: 'onChange',
  });
  ...
  const {
    ...
    () => MaterialPageApi.getMaterialDetail(id),
    {
      enabled: !!id,
      cacheTime: 300000,
      staleTime: 300000,
  ...
  const { mutate: submitForm, isLoading: loadingSubmit } = useMutation(
    (variable: IFormMaterial) => {
      return MaterialPageApi.createMaterial(variable);
    },
    {
      onSuccess: (res) => {
        if (res?.code === 200) {
  ...
  const handleSubmitForm = (values: IFormMaterial): void => {
    if (loadingSubmit) return;
    submitForm(values);
  };
